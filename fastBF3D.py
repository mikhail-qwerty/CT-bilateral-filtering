...
def bilateral_numpy_fast_3D(image, sigmaspatial, sigmarange, samplespatial=None, samplerange=None):
    ...
    (zgrid, ygrid, xgrid) = np.meshgrid(range(zdim), range(ydim), range(xdim), indexing='ij')
    ...
    (zgrid, ygrid, xgrid) = np.meshgrid(np.arange(zdim), np.arange(ydim), np.arange(xdim), indexing='ij')
